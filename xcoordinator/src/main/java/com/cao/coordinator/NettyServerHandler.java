 ...
 * 4. 通知各个子事务提交或回滚(事务参与者需要先Hold住本地事务，等待事务管理者的命令才可以进行下一步)
 */
...
public class NettyServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public synchronized void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        String command = jsonObject.getString("cmd"); // create-创建事务组，add-添加事务
        ...
        if ("create".equals(command)) {
            ...
            if (isEndMap.get(groupId) && transactionCountMap.get(groupId).equals(transactionTypeMap.get(groupId).size())) {
                if (transactionTypeMap.get(groupId).contains("rollback")){
                    result.put("cmd", "rollback");
                    sendResult(result);
                } else {
                    result.put("cmd", "commit");
                    sendResult(result);
                }
            }
